		...
		try {
			...
			final var generated = client.symmetricGenerate(
					SymmetricGenerateRequest
							.builder().algorithm(SymmetricAlgorithm.AES256_FF3_1).purpose(KeyPurpose.FPE)
							.name("java-fpe-example-" + time)
							.build());
			...
			final var encrypted = client.encryptTransform(
					EncryptTransformRequest
							.builder().id(keyId).plainText(plainText).alphabet(TransformAlphabet.NUMERIC)
							.tweak(tweak)
							.build());
			...
			final var decrypted = client.decryptTransform(
					DecryptTransformRequest
							.builder().id(keyId).cipherText(encryptedText).tweak(tweak)
